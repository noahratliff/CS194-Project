...
class ModelSuitability:
    ...
    def compute_similarities(self, new_question_embedding):
        ...
        similarities = np.array([
            np.dot(new_question_embedding, emb) / (np.linalg.norm(new_question_embedding) * np.linalg.norm(emb))
    ...
    def suitability_score(self, new_question):
        new_question_embedding = self.model.encode(new_question, convert_to_tensor=False)
